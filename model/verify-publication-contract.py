...
def main() -> None:
    ...
    require(contract["status"] == "provisional-publication-strategy", "publication strategy must remain explicitly provisional before activation")
    ...
    require(term["activationStatus"] == "planned-not-active", "permanent namespace must not be claimed active before the resolver exists")
    ...
    require(bool(re.match(r"^(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)$", release["initialRepositoryVersion"])), "initial repository version is not SemVer")
    ...
    require([module["name"] for module in modules] == ["core", "capability", "service", "agent", "deployment"], "module publication order/identity changed unexpectedly")
    ...
    for module in modules:
        ...
        require(module["currentOntologyIri"].startswith("urn:eska:model:"), f"{path}: current ontology IRI is no longer provisional")
        require(module["targetOntologyIri"] == f"https://w3id.org/eska/model/{module['name']}", f"{path}: unexpected target ontology IRI")
        require(bool(re.match(r"^(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)$", module["firstPublishedVersion"])), f"{path}: first published version is not SemVer")
        require("https://w3id.org/eska" not in text, f"{path}: permanent namespace was introduced before activation")
    ...
    # The namespace migration is intentionally one-to-one by local name.
    migration_pairs = {
    ...
    print("SUCCESS: ESKA namespace/publication/versioning strategy is machine-verifiable without prematurely activating the permanent namespace.")
